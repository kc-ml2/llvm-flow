...
def save_file_to_s3(file: str, save_path: str) -> str:
    ...
    bucket = settings.S3_BUCKET_SAFE
    s3 = boto3.client("s3")

    with open(file, "rb") as f:
        s3.upload_fileobj(f, bucket, save_path)
...
def save_directory_to_s3(directory_path: str, s3_key: str) -> str:
    ...
    for filepath in directory.rglob("*"):
        if filepath.is_file():
            ...
            logger.debug(f"Uploaded {filepath} to s3://{bucket}/{s3_key}")

    logger.debug(f"All files uploaded from {directory_path} to s3://{bucket}/")
...
def download_directory_from_s3(save_path: str, download_path: str) -> str:
    ...
    pages = paginator.paginate(Bucket=bucket, Prefix=save_path)
